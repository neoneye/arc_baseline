...
if False:
    with open(SUBMISSION_FILENAME, 'w') as outfile:
        outfile.write(solution_json)


import pandas as pd
df_sub = pd.DataFrame(kaggle_output_ids, columns=['output_id'])
df_sub["output"] = ""
df_sub = df_sub.sort_values("output_id").reset_index(drop=True)
print(f"Writting data to {SAMPLE_SUBMISSION} ...")
df_sub.to_csv(SAMPLE_SUBMISSION, index=None)
